...
const GIFT_CARD_SEED = new TextEncoder().encode('gift_card')
...
/** Gift cards are keyed by sender + the item's database UUID, one PDA per thing inside */
export function findGiftCardAddress(sender: PublicKey, cardId: string): PublicKey {
  return PublicKey.findProgramAddressSync(
    [GIFT_CARD_SEED, sender.toBytes(), uuidToBytes(cardId)],
    MORROW_PROGRAM_ID,
  )[0]
}
...
export const DISCRIMINATORS = {
  ...
  createGiftCard: [202, 141, 181, 51, 5, 224, 200, 17],
  claimGiftCard: [154, 90, 42, 127, 74, 206, 189, 80],
  refundGiftCard: [101, 162, 142, 147, 151, 148, 210, 99],
  ...
  giftCardAccount: [54, 20, 8, 38, 75, 170, 131, 139],
