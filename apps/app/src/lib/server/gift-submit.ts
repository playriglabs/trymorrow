...
import { GIFT_COLUMNS, GIFT_LIFETIME_DAYS, type GiftRow, giftLabel } from '@/lib/server/gifts'
...
/**
 * Broadcasts a signed gift transaction. The server decides what it does by reading the
 * instructions itself: exactly one action per stock in the gift, all the same kind, and nothing
 * else but the recorded fee. Only then does the gift's status move forward. The browser's signature
 * and a tip's policy-bound signature both come through here, so neither gets a looser check.
 */
export async function submitGiftTransaction(
  gift: GiftRow,
  signedTransaction: string,
  viewer: UserRow,
  request: Request,
): Promise<GiftRow> {
  const transaction = parseRelayedTransaction(signedTransaction)
