  ...
  const share = () => {
    if (canShare()) {
      navigator
        .share({
          title: 'A gift idea',
          text: match({ stock, wishCash })
            .with({ stock: P.nonNullable }, ({ stock }) => `I'd love a piece of ${stock.name}`)
            .with({ wishCash: true }, () => `I'd love ${formatUsd(usd)} in cash`)
            .otherwise(() => 'Send me a gift on Morrow'),
          url: link,
        })
        .catch(() => {})
      return
  ...
  return (
    <Screen
      ...
      footer={
        <Button onClick={share}>
          ...
          {match({ sharing: canShare(), copied })
            .with({ sharing: true }, () => 'Share the ask')
            .with({ copied: true }, () => 'Link copied')
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        ...
        {match({ stock, wishCash })
          .with({ stock: P.nonNullable }, ({ stock }) => (
